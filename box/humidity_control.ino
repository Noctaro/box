...
if (zaehler == Messdurchgaenge)                                  //Wenn der Messzähler Messdurchgaenge Messdurchgänge erreicht hat                                  
 {
  ...
  //TESTING BEFEUCHTUNG BEIM LÜFTEN
    ...
    if(water_with_air == 1 && t >= minTemperatur && h < optimaleLuftfeuchte) // Wenn die temperatur größer oder gleich der minimalen Temperatur ist und die Luftfeuchte kleiner als die optimale Feuchte ist 
      {
      relais_1_on();              //Schalte befeuchtung ein
      Absaugung_aktiv = 1;
      }
    
    if(water_with_air == 1 && h >= maxLuftfeuchte) //Wenn die Luftfeuchte größer oder gleich der maximalen Feuchte ist
      {
